...
connectDB().then(() => {
  ...
  Club.countDocuments().then(count => {
    if (count === 0) {
      ...
      Club.insertMany(initialClubs)
        .then(() => console.log('Successfully seeded clubs data!'))
        .catch(err => console.error('Error seeding clubs data:', err));
    }
  });
});
